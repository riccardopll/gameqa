...
const bridge = async <T>(job: RunnerJob, pathName: string, body: unknown) => {
    ...
    if (!response.ok) {
        throw new Error(
            payload.error ?? `Bridge ${pathName} failed with ${response.status}`,
        );
    }
...
const applyDecision = async (page: Page, decision: AgentDecision) => {
    ...
    const result = await page.evaluate(async (nextDecision) => {
        ...
        if (nextDecision.type === "controller_action") {
            return target.__GAMEQA_AGENT__.applyControllerAction(
                nextDecision.actionId ?? "",
            );
        }
...
const hasWork = (inspection: SdkInspection) =>
    Boolean(
        ...
        (inspection.driver &&
            (inspection.driver.goals.length > 0 ||
                inspection.driver.scenarios.length > 0)),
    );
