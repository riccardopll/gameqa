...
const getInjectedSession = () => {
    const parsed = localSessionSchema.safeParse(
        (globalThis as RuntimeGlobal).__GAMEQA_SESSION__,
    );
...
export const createClient = () => {
    ...
    const inspect = async () => {
        ...
        const driver = state.driver
            ? {
                  observation: driverObservationSchema.parse(
                      await state.driver.observe(),
                  ),
        ...
        target.__GAMEQA_AGENT__ = {
            ...
            runDriverGoal: async (goal, args = {}) => {
                ...
                try {
                    ...
                    const message =
                        error instanceof Error ? error.message : "Driver goal failed";
